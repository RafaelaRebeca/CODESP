# Modelo de Domínio

## Histórico de Revisões

| Data | Versão | Descrição | Autores |
| :--: | :----: | :-------: | :-----: |
| - | - | Versão inicial |  - |
| - | - | - |  - |

## 1. Diagrama das Classes Conceituais do Domínio

![Substitua pela imagem do diagrama de classes de domínio...](/doc/dominio/imgs/dominio.png)

[LINK para o arquivo Astah com o modelo](/doc/arquivo_astah/CODESP.asta)

## 2. Glossário (sugestão)

**Superclasse** : Usuário

*Descrição*: 

| Termo | Explicação |
| :---: | :--------: |
| nome | Explicação 1... |
| matricula ||
| E-mail | Explicação 1... |
| Senha | Explicação 2... |
| Tipo | Explicação 3...|

---

**Subclasse** : Estudante

*Descrição*: 

| Termo | Explicação |
| :---: | :--------: |
| genero | Explicação 2... |
| cpf ||
| telefone ||
| data_nascimento ||
| nee ||

---

**Subclasse** : Gerente

*Descrição*: 

| Termo | Explicação |
| :---: | :--------: |
| cargo | Explicação 1... |

---

**Classe associativa** : Inscrição (Estudante-Turma)

*Descrição*: 

| Termo | Explicação |
| :---: | :--------: |
| data | Explicação 1... |
| status | Explicação 2... |

---

**Classe associativa** : Turma

*Descrição*: 

| Termo | Explicação |
| :---: | :--------: |
| nome | Explicação 1... |
| modalidade | Explicação 2... |
| horario | Explicação 2... |
| professor | Explicação 2... |
| local | Explicação 2... |
| vagas | Explicação 2... |
| status | Explicação 2... |

---

**Classe regular** : Lista de espera

*Descrição*: 

| Termo | Explicação |
| :---: | :--------: |
| - | - |
